...
def get_line(linename, Z, age, OH):
    ...
    line = file["/PartType0/" + linename][:]

    lbin, rbin, _ = stats.binned_statistic(r, line, statistic="mean", bins=100)
...
for linename in lines:
    ...
    for iZ in range(len(Zs)):
        ...
        for iage in range(len(ages)):
            ...
            for iOH in range(len(OHs)):
                OH = OHs[iOH]
                r, line = get_line(linename, Z, age, OH)
                ax[iZ][iage].semilogy(
                    r, line, "-", color=colors[iOH], label="${0}$".format(OH)
                )
    ...
    for i in range(4):
        ax[i][0].set_ylabel(
            "{0} (J m$^{{-3}}$ s$^{{-1}}$)".format(linename.replace("_", "\_"))
        )
    for i in range(5):
    ...
    pl.savefig("line_{0}.png".format(linename), dpi=300, bbox_inches="tight")
